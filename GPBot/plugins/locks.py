...
@Stark.on(events.NewMessage(pattern="^[!?/]lock ?(.*)"))
@is_admin
...
@Stark.on(events.NewMessage(pattern="^[!?/]unlock ?(.*)"))
@is_admin
...
@Stark.on(events.NewMessage(pattern="^[!?/]locktypes"))
...
@Stark.on(events.callbackquery.CallbackQuery(data="locks"))
async def _(event):
