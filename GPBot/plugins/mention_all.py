...
async def all(event):
    ...
    if event.is_private:
        return await event.respond("This command Can Be Use In Groups.")
    ...
    usrnum = 0
    usrtxt = ""
    async for usr in Stark.iter_participants(chat_id):
        ...
        if usrnum == 10:
            if mode == "text_on_cmd":
                txt = f"{usrtxt}\n\n{msg}\n\nMeet Me Here🙈 @JackSparrowSupport ✨🥀"
                await Stark.send_message(chat_id, txt)
...
@Stark.on(events.NewMessage(pattern="^/cancel$"))
